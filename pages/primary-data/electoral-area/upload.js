...
import UploadElectoralArea from "../../../components/primary-data/UploadElectoralArea";
      ...
      <div className="main-content">
        <div className="page-content">
          <div className="container-fluid">
            <UploadElectoralArea data={data} districts={districts}/>
          </div>
        </div>
      </div>
    </div>
...
export async function getServerSideProps(context) {
  ...
  const data = await fetch(`${SERVER_BASE_URL}/api/v1/primary-data/location/electoral-area?session=${session}&page=${page}&searchText=${searchText}`).then(
    (res) => res.json()
  );
